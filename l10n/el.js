OC.L10N.register(
    "souvenirs",
    {
    "Souvenirs" : "Αναμνηστικά",
    "Browse and view your \"souvenirs\" albums on nextcloud web interface" : "Περιηγηθείτε και δείτε τα \"αναμνηστικά\" άλμπουμ σας στη διεπαφή ιστού του nextcloud",
    "View and share albums made by the [Souvenirs android application](https://github.com/zorgluf/souvenirs-android).\n    \\nThis application is just a viewer. No album creation or edition features are available for the moment. You will need to create albums using the android app and sync them to your nextcloud server." : "Προβάλετε και μοιραστείτε άλμπουμ που έχουν δημιουργηθεί από την εφαρμογή [Souvenirs android](https://github.com/zorgluf/souvenirs-android).\n    \\nΑυτή η εφαρμογή είναι απλώς για προβολή. Δεν υπάρχουν διαθέσιμες λειτουργίες δημιουργίας άλμπουμ ή έκδοσης προς το παρόν. Θα χρειαστεί να δημιουργήσετε άλμπουμ χρησιμοποιώντας την εφαρμογή android και να τα συγχρονίσετε με τον διακομιστή nextcloud.",
    "Start slideshow" : "Έναρξη παρουσίασης",
    "Stop slideshow" : "Διακοπή παρουσίασης",
    "Slideshow speed" : "Ταχύτητα παρουσίασης",
    "Download" : "Λήψη",
    "Fullscreen" : "Πλήρης οθόνη",
    "Click to download album in a zip file." : "Κάντε κλικ για λήψη του άλμπουμ σε ένα αρχείο zip."
},
"nplurals=2; plural=(n != 1);");
